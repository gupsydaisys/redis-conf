...
def get_credentials(email, pwd):
    ...
    r = redis.StrictRedis(host=redis_hostname, port=redis_port, password=pwd)
    try:
        for db_type in ['ft', 'rebloom', ]:
...
def get_user_input():
    ...
    email = ""
    while len(email) == 0:
        email = raw_input("Please enter your registration email: ")
    
    pwd = ""
    while len(pwd) == 0:
        pwd = raw_input("Please enter the password from the instructor: ")
...
def main():
    ...
    print("For this training you will need the RediSearch (ft) and ReBloom modules: \n")
